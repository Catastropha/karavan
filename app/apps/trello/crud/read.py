...
async def get_card(card_id: str) -> CardOut:
    ...
    resp = await res.trello_client.get(f"cards/{card_id}", params=_auth_params())
...
async def get_list_cards(list_id: str) -> list[CardOut]:
    ...
    resp = await res.trello_client.get(f"lists/{list_id}/cards", params=_auth_params())
    resp.raise_for_status()
...
async def get_board_lists(board_id: str) -> list[ListOut]:
    ...
    resp = await res.trello_client.get(f"boards/{board_id}/lists", params=_auth_params())
    resp.raise_for_status()
